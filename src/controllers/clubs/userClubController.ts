...
const createAndAdministrateClub = async (req: Request, res: Response) => {
  ...
  try {
    ...
    return res.status(500).json({
      ...
      details:
        "There was an internal server error while creating the club administration relationship. Please try again later.",
...
const inviteAdmin = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (exists) {
      return res
        .status(409)
        .json({ error: "User already an admin for that club" });
    }
...
export { createAndAdministrateClub, inviteAdmin, removeAdmin };
